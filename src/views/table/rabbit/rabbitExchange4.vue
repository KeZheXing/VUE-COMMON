<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-waves class="filter-item" type="primary" @click="handleFilter2">{{ $t('table.out') }}</el-button>
      <el-button v-waves class="filter-item" type="primary" @click="handleFilter3">导出未提取</el-button>
      <el-button v-waves class="filter-item" type="primary" @click="del">删除已提取</el-button>
      <el-button
        class="filter-item"
...
<!--      <el-button v-waves class="filter-item" type="success" @click="bindPhone">安全绑定</el-button>-->
    ...
    >
      ...
      <el-table-column v-if="checkPermission(['ADMIN'])" label="密码" width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.password }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="$t('table.money')" min-width="80">
        <template slot-scope="scope">
      ...
      <el-table-column label="用户类型" min-width="80">
        <template slot-scope="scope">
          <el-tag>{{ scope.row.userType=='ADMIN'?"管理员": scope.row.userType=='AGENT'?"代理":"普通用户" }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.pnickname')" min-width="80">
        <template slot-scope="scope">
          <el-tag>{{ scope.row.pnickname }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="消费金额(新)" min-width="80">
        <template slot-scope="scope">
          <el-tag>{{ (scope.row.totalcost==(''||null))?0:scope.row.totalcost }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column label="累计充值" min-width="80">
        <template slot-scope="scope">
          <el-tag>{{ (scope.row.allinvest==(''||null))?0:scope.row.allinvest }}</el-tag>
        </template>
      </el-table-column>
      ...
      <!--      <el-table-column :label="$t('table.i')" min-width="150px">-->
      <!--        <template slot-scope="scope">-->
      <!--          <el-tag>{{ scope.row.isPickup==true?"已提取":"未提取" }}</el-tag>-->
      <!--        </template>-->
      <!--      </el-table-column>-->
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          <!--          <el-button type="primary" size="mini" @click="handleUpdate(scope.row)">{{ $t('table.copy') }}</el-button>-->
          <!--          <el-button v-if="scope.row.wxData" size="mini" type="success" @click="handleModifyStatus(scope.row,'published')">-->
          <!--            {{ $t('table.publish') }}-->
          <!--          </el-button>-->
          <el-button size="mini" round @click="toinvest(scope.row.id)">充值</el-button>
          <el-button v-if="checkPermission(['ADMIN'])" size="mini" round @click="updateByp(scope.row.id)">修改</el-button>
          <el-button v-if="checkPermission(['ADMIN'])" size="mini" round @click="disable(scope.row.id)">{{
            scope.row.isActive==false?"启用":"禁用" }}
          </el-button>
          <el-button v-if="checkPermission(['ADMIN'])" size="mini" round @click="del(scope.row.id)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form>
        ...
        <el-form-item :label="$t('table.name')" prop="nickname">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogStatus=='createUser'?createUser():createAgent()">{{ $t('table.confirm')
        }}
        </el-button>
      </div>
    </el-dialog>

    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisiblebind">
      <el-form>
...
export default {
  ...
  data() {
    return {
      reset: '读取中',
      fileList: [],
      typelist: [{ 'key': undefined, 'display_name': '全部' }, { 'key': 'USER', 'display_name': '普通用户' }, {
        'key': 'AGENT',
        'display_name': '代理用户'
      }],
  ...
  methods: {
    ...
    toinvest(userId) {
      this.$prompt('请输入充值金额', '提示', {
        ...
        inputErrorMessage: '输入的金额不正确'
      }).then(({ value }) => {
        invest({ id: userId, money: value }).then(res => {
          this.$message({
            type: 'success',
    ...
    disable(userId, active) {
      MessageBox.confirm(active ? '禁用后用户将失去部分功能！请谨慎操作！' : '启用后用户功能将重新开启', {
        confirmButtonText: active ? '立即禁用' : '立即启用',
        cancelButtonText: '取消',
        type: 'success'
      }).then(() => {
        try {
          disable({ id: userId }).then(res => {
            if (res.data.val === 200) {
              MessageBox.confirm(active ? '禁用成功' : '启用成功', {
                confirmButtonText: '确认',
                cancelButtonText: '取消',
                type: 'success'
              }).then(() => {
    ...
    handleCreate2() {
      ...
      getBindPhone().then(res => {
        if (res.data.val === 200) {
          ...
          this.dialogFormVisiblebind = true
          this.$nextTick(() => {
            this.$refs['dataForm'].clearValidate()
          })
        }
    ...
    toBindPhone() {
      toBindPhone({ phone: this.temp.phone }).then(res => {
        if (res.data.val === 200) {
          this.$message({
            type: 'success',
            message: '更新绑定信息'
