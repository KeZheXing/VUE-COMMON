...
<!--      </el-table-column>-->
      ...
      <el-table-column :label="$t('table.type')" min-width="150px">
        <template slot-scope="scope">
          <el-tag>{{ scope.row.type}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.auto_delete')" min-width="150px">
        <template slot-scope="scope">
          <el-tag>{{ scope.row.auto_delete}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.durable')" min-width="150px">
        <template slot-scope="scope">
          <el-tag>{{ scope.row.durable}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.internal')" min-width="150px">
        <template slot-scope="scope">
          <el-tag>{{ scope.row.internal}}</el-tag>
        </template>
      </el-table-column>
...
<script>
import { getExchange } from '@/api/rabbit'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getExchange(this.listQuery).then(response => {
        this.listLoading = false
        this.list = response.data.items
        this.total = response.data.filtered_count
