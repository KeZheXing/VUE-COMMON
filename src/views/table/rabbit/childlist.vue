<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate">添加子账户</el-button>
    ...
    >
      ...
      <el-table-column label="提取次数" min-width="80">
        <template slot-scope="scope">
          <el-tag>{{ (scope.row.todayNumber==(''||null))?0:scope.row.todayNumber }}</el-tag>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button size="mini" round @click="updateByp(scope.row.id)">修改</el-button>
          <el-button size="mini" round @click="disable(scope.row.id,scope.row.isActive)">{{ scope.row.isActive==false?"启用":"禁用" }}
          </el-button>
          <el-button size="mini" round @click="del(scope.row.id)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form>
        ...
        <el-form-item :label="$t('table.username')" prop="用户名前缀">
          <el-input v-model="nickname" placeholder="用户名前缀"  :disabled="true" />
        </el-form-item>
...
import { getTokenByName } from '@/utils/auth' // get token from cookie
import { getExchange, invest, del, disable, createChild, createAgent, updateByp } from '@/api/member'
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.resetTemp()
      this.nickname = getTokenByName('username')
