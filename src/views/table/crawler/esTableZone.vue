<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.poicount')" min-width="150px">
        <template slot-scope="scope">
...
<script>
import { getzone, crawlerPoi } from '@/api/estable'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getzone(this.listQuery).then(response => {
    ...
    handleModifyStatus(row) {
      this.listLoading = true
      crawlerPoi({ name: row.name }).then(response => {
        this.listLoading = false
        this.$message({
